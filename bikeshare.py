...
def get_filters():
    ...
    while True:
        city = input('Please select a city: Chicago, New york, or Washington\n')
        if city.lower() in ['chicago', 'new york', 'washington', 'c']:
    ...
    while True:
        month = input('Please enter a month between January and June or "all" for all months:\n')
    ...
    while True:
        ...
        if len(day_in) > 1 and [i for i in days_list if day_in in i] :
            day = [i for i in days_list if day_in in i[0:3]]
            print('You have chosen {}.\n'.format(str(day).title().strip("[]'")))
            break
        else:
            print('Invalid input. Try again.\n\n')
